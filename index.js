...
app.use(
  cors({
    origin: ["http://localhost:3000", "https://ai-food-creator.netlify.app"],
  })
);
...
function validateRequest(body) {
  ...
  if (
    ...
    (!Number.isInteger(body.serves) || body.serves <= 0)
  )
    return "Missing or invalid serves";
...
app.post("/generate-recipe", async (req, res) => {
  ...
  let preferences = Array.isArray(req.body.preferences)
  ...
  try {
    ...
    if (isUnexpected(response)) {
      ...
      return res.status(500).json({
        error: response.body.error?.message || "Azure ModelClient error.",
    ...
    try {
      ...
      console.error(
        "Failed to parse recipe from AI response:",
    ...
    if (
      ...
      return res
        .status(500)
        .json({ error: "Incomplete recipe data from AI.", raw: recipe });
    }
    ...
    res.status(500).json({
      error: "Failed to generate recipe. " + (err.message || err.toString()),
      details: err,
    });
  }
});
